...
class BooleanOperationsProperSubset(Scene):
    def construct(self):
        ellipse1 = Ellipse(
            width=2.5, height=2.5, fill_opacity=0.5, color=BLUE, stroke_width=10
        ).move_to(LEFT)
        bool_ops_text = MarkupText("<u>Proper Subset</u>").next_to(ellipse1, UP * 3)
        ...
        subset1_text = MathTex(r"B \not\subset A", font_size=36).next_to(ellipse2, UP)
        ...
        subset3_text = MathTex(r"C \subset A", font_size=36).next_to(ellipse3, UP)
...
# manim -pql --format gif --fps 30 bool_ops_Propersubset.py BooleanOperationsProperSubset