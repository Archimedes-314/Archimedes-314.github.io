...
{\Huge\scshape Syntax \& Semantics (Simple)\par}
\vspace{0.3cm}

{\large Topic 1, Section 1, lesson 1\par}

\vspace{1.2cm}

{\LARGE\bfseries Mathematical \\[0.3em]
\large Notations and Terminology\par}
\vfill

\begin{minipage}{0.7\textwidth}
\centering
\small\itshape
`To ask the right question is already half the solution to a problem.'\\[0.3cm]
— C.\,G. Jung
\end{minipage}
...
\sssu{Propositions}
A \textit{proposition} is a declarative statement that is either true or false, but not both. 

\textit{Propositional logic} focuses on the relationships between these propositions and how they can be combined using logical operators.

\sssu{Sets}
A \textit{set} is a collection of objects considered as a single entity. The objects in a set are called its \textit{elements} or \textit{members}. 

One common way to formally define a set is to list its elements inside curly brackets. We can use \textit{set-builder notation} to describe a set containing elements according to some rule, which is often in the form \(\{n : [\text{rule about } n]\}\). 

The order in which we list the elements does not matter, and repeating elements does not change the set. If we want to keep track of how many times each element appears, we use the term \textit{multiset} instead of set.
...
\begin{table}[H]
    \centering
    \begin{tabular}{|>{\centering\arraybackslash}p{2.5cm}|p{5.5cm}|p{6cm}|}
        \hline 
        \textbf{Symbol} & 
        \textbf{What is Means} & 
        \textbf{Notes}
        ...
        \(P \land Q\) & 
        \(P\) and \(Q\) & 
        \(\land\) looks like a capital A for `and'.
        \\\hline
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        \(P \lor Q\) 
        & \(P\) or \(Q\) (inclusive) 
        & If either one or both of are true, the statement \(P \lor Q\) is true.
        \\\hline
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        \(P \rarrow Q\) 
        & \(P\) implies \(Q\) 
        ...
        \makecell[l]{
            ...
            \((P~\rarrow~Q)~\land~(Q~\rarrow~P)\) \\
            \((P~\land~Q)~\lor~(\neg~P~\land~\neg~Q)\)
        }&
        `If and only if' is commonly shortened to `iff'.
        \\\hline
    \end{tabular}
\end{table}
This table will be expanded and improved overtime.

\newpage

\section{Set-theoretic Symbols}

\begin{table}[H]
    \centering
    \begin{tabular}{|>{\centering\arraybackslash}m{2.5cm}|m{5cm}|>{\centering\arraybackslash}m{4cm}|}
        \hline\textbf{Symbol} & \textbf{Meaning} & \textbf{Example}\\\hline
        
        \(\forall\) & 
        For all values of \(x\) & 
        \(\forall x (x \in \R \rarrow x^2 \in \R)\) 
        \\\hline
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        \(\exists\) & 
        There exists at least one value & 
        \(\exists x (x \in \R \land x^2 = 4)\) 
        \\\hline
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        \(\in\) & 
        Is an element of & 
        ...
        \makecell[l]{
            Is a subset of \\ 
            \( S \subseteq T \ioi \forall x(x \in S \rarrow x \in T)\)
        } &
        \makecell[c]{
            \(\{3,7\}~\subseteq~\{3,5,7\}\) \\
            \( \{3,5,7\}~\subseteq~\{3,5,7\} \)
        }
        \\\hline
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        \( \nsubseteq \) &
        \makecell[l]{
            Is not a subset of \\
            \(S \nsubseteq T \ioi \exists x (x \in S \land x \notin T)\)
        } &
        \(\{1,2,3\} \nsubseteq \{1, 3, 5, 7, 9\}\)
        \\\hline
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        \makecell[l]{
            ...
            \(S \subset T \ioi (S \subseteq T \land S \neq T)\)
        } &
        \(\{1, 3, 5\} \subset \{1, 3, 5, 7, 9\}\)
        \\\hline
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        \(\nsubset\) &
        \makecell[l]{
