...
\begin{itemize}
    ...
    \item \(r \land w\): It is raining and the grass is wet.
    \item \(s \lor r\): The sun is shining or it is raining.
    \item \(\neg (r \lor s) \rarrow c\): It is neither raining nor is the sun shining, which means it must be cloudy.
\end{itemize}
...
\section{The Set of Real Numbers (and its Subsets).}
...
\egblueboxs{QuantifersEg}{
    ...
    \begin{enumerate}
        ...
        \item If \(x \notin \Z\), then \(n \in \Z\) is false. So the implication is true for all  \(n \in \R \setminus \Z\), and thus every non-natural real \(x\) is included. If \(x \in \N\), then we need \(x = n \rarrow x = n^2\). This is only true for \(0\) and \(1\).
